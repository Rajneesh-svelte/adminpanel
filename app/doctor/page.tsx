...
export default function Doctor() {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      ...
      try {
        ...
        const response: { data: ApiDoctorData[] } = await apiClient.get(
          'https://beta.homeivf.in/appointment/api/v1/doctor_list/'
        );
  ...
  useEffect(() => {
    ...
    const found = doctors.find(
      (d) => d.name?.toLowerCase().trim() === String(patient.assigned_doctor).toLowerCase().trim()
    );
    if (found) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const url = `https://beta.homeivf.in/admin_dashboard/api/v1/find-patient/?uhid=${encodeURIComponent(
        searchTerm.trim()
      )}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = 'https://beta.homeivf.in/admin_dashboard/api/v1/change-doctor/';
      const payload = {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="max-w-2xl mx-auto mb-8">
        <h1 className="text-2xl font-semibold text-gray-800 text-center mb-6">Patient Search</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                ...
                {patientError && !isSearchingPatient && (
                  <div className="text-red-600 text-sm">{patientError}</div>
                )}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                {isLoadingDoctors && (
                  <div className="text-gray-600 text-sm">Loading doctors...</div>
                )}
                {!isLoadingDoctors && doctorsError && (
                  <div className="text-red-600 text-sm">{doctorsError}</div>
                )}
              ...
              {(changeDoctorError || changeDoctorSuccess) && (
                <div
                  className={`mt-4 text-sm ${
                    changeDoctorError ? 'text-red-600' : 'text-green-700'
                  }`}
                >
                  {changeDoctorError || changeDoctorSuccess}
                </div>
              ...
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-md transition-colors text-white ${
                    isChangingDoctor ? 'bg-blue-300' : 'bg-blue-600 hover:bg-blue-700'
                  }`}
                >
